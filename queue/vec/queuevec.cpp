...
// Copy constructor
template <typename Data>
QueueVec<Data>::QueueVec(const QueueVec<Data> &q): Vector<Data>::Vector(q), head(q.head), tail(q.tail), elNums(q.elNums) {}

// Move constructor
template <typename Data>
QueueVec<Data>::QueueVec(QueueVec<Data> &&q) noexcept : Vector<Data>::Vector(std::move(q)) {
